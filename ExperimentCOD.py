...
# =========================
# Pentru Unicorn Recorder, setările implicite sunt de obicei:
# IP = 127.0.0.1 și Port = 1000
# Modifică doar dacă în Recorder sunt alte setări.
UDP_IP = "127.0.0.1"
UDP_PORT = 1000
udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
udp_endpoint = (UDP_IP, UDP_PORT)

# Coduri de marker (un singur caracter ASCII)
MARKERS = {
    "practice_standard": 1,
    "practice_target": 2,
    "oddball_standard": 1,
...
def send_udp_trigger(code):
    """
    Trimite triggerul ca ASCII prin UDP către Unicorn Recorder.
    Exemplu: 1 -> b"1"
    """
    msg = str(code).encode("ascii")
    udp_sock.sendto(msg, udp_endpoint)


def send_trigger_on_flip(win, code):
    """
    Programează trimiterea triggerului exact la următorul flip de ecran.
    """
    win.callOnFlip(send_udp_trigger, code)
...
def run_oddball_practice(win, writer, fieldnames, participant_code, n_targets=3):
    ...
    for trial_index, trial in enumerate(practice_trials, start=1):
        ...
        if acc == 1:
            ...
            if trial["correct_response"] == 1:
                fb_text = "Trebuia să apăsați SPACE"
            else:
                fb_text = "Nu trebuia să răspundeți"
...
def run_lpp_block(win, writer, fieldnames, participant_code, csv_path):
    ...
    if counts != {"positive": 30, "neutral": 30, "negative": 30}:
        raise ValueError(
